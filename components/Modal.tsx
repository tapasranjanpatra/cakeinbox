...
//   cake: Cake;
//   closeModal: () => void;
//   addToCart: (cake: Cake) => void; // Updated to pass the cake object
// };

// const Modal: React.FC<ModalProps> = ({ cake, closeModal, addToCart }) => {
//   console.log("Modal props:", { cake, closeModal, addToCart }); 
//   const modalRef = useRef<HTMLDivElement>(null);
...
//         <p className="mb-4 text-lg text-gray-600">{cake.description}</p>
//         <button
//           onClick={() =>{ addToCart(cake)}} // Pass cake object
//           className="w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300"
//         >
//           Add to Cart
//         </button>
...
//   cake: Cake;
//   closeModal: () => void;
//   addToCart: (cake: Cake) => void; // Define the type for addToCart
// };
...
//         <p className="mb-4 text-lg text-gray-600">{cake.description}</p>
//         <button
//           onClick={() => addToCart(cake)} // Pass cake object to addToCart
//           className="w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300"
//         >
//           Add to Cart
//         </button>
...
const Modal: React.FC<ModalProps> = ({ cake, closeModal, addToCart }) => {
  ...
  const handleAddToCart = () => {
    addToCart(cake);  // Add the selected cake to the cart
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div ref={modalRef} className="relative bg-white p-8 rounded-lg shadow-lg max-w-md mx-auto transition-transform transform duration-300 scale-95 hover:scale-100">
        ...
        <p className="mb-4 text-lg text-gray-600">{cake.description}</p>
        <button
          onClick={handleAddToCart}  // Use the new function that adds to cart and closes the modal
          className="w-full px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300"
        >
          Add to Cart
        </button>
